--- Hongtu Gateway helper methods to send HTTP responses to clients.
...
local server_header = "FFan.Hongtu.Gateway.V1.0"
...
-- @field HTTP_NOT_ALLOWED 405 Method Not Allowed
...
-- @field status_codes.HTTP_NOT_ALLOWED Always "Method not allowed"
local response_default_content = {
  ...
  [_M.status_codes.HTTP_NOT_ALLOWED] = function(content)
...
local function send_response(status_code)
  ...
  return function(content, headers)
    ...
    local encoded, err
    if content then
      encoded, err = cjson.encode(type(content) == "table" and content or {status = status_code, message = content, data = nil})
      if not encoded then
