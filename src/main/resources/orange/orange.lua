...
local function load_node_plugins(config, store, redis)
    ...
    for _, v in ipairs(plugins) do
        ...
        if not loaded then
            ...
            table_insert(sorted_plugins, {
                name = v,
                handler = plugin_handler(store,redis,config)
            })
        end
    end
...
function Orange.init(options)
    ...
    local status, err = pcall(function()
        ...
        redis = require("orange.store.redis_store")(config.store_redis)

        loaded_plugins = load_node_plugins(config, store, redis)
        ngx.update_time()
...
function Orange.proxy(accessFlag)
    ngx.ctx.ORANGE_REDIRECT_START = now()

    for _, plugin in ipairs(loaded_plugins) do
        plugin.handler:proxy(accessFlag)
    end
