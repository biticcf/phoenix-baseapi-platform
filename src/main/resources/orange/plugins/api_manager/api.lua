...
local API = BaseAPI:new("api-manager-api", 2)

API:post("/api_manager/enable", function(store)
    return function(req, res, next)
        ...
        local api_manager_enable = "0"
        if enable then api_manager_enable = "1" end
        local update_result = store:update({
            sql = "replace into meta SET `key`=?, `value`=?",
            params = { "api_manager.enable", api_manager_enable }
        })

        if update_result then
            local success, err, forcible = orange_db.set("api_manager.enable", enable)
            result = success
        else
            result = false
        end
        ...
        if result then
            res:json({
                success = true,
                msg = (enable == true and "开启api_manager成功" or "关闭api_manager成功")
            })
        else
            res:json({
                success = false,
                data = (enable == true and "开启api_manager失败" or "关闭api_manager失败")
            })
        end
    end
end)

API:get("/api_manager/fetch_config", function(store)
    return function(req, res, next)
        local data = {}
...
API:post("/api_manager/sync", function(store)
    return function(req, res, next)
        local data = {}
        -- 查找enable
        local enable, err1 = store:query({
        ...
        local ss, err3, forcible = orange_db.set("api_manager.enable", data.enable)
        ...
        ss, err3, forcible = orange_db.set_json("api_manager.conf", data.conf)
        if not ss or err3 then
            return res:json({
                success = false,
...
API:get("/api_manager/configs", function(store)
    return function(req, res, next)
        res:json({
            success = true,
            data = {
                enable = orange_db.get("api_manager.enable"),
                conf = orange_db.get_json("api_manager.conf")
            }
        })
    end
end)
...
-- new
API:post("/api_manager/configs", function(store)
    return function(req, res, next)
        ...
        local update_result = store:update({
            sql = "replace into meta SET `key`=?, `value`=?",
            params = { "api_manager.conf", conf }
        })

        if update_result then
            local result, err, forcible = orange_db.set("api_manager.conf", conf)
            success = result
            if success then
                data.conf = cjson.decode(conf)
                data.enable = orange_db.get("api_manager.enable")
            end
        else
            success = false
        end
 ...
 -- modify
API:put("/api_manager/configs", function(store)
    return function(req, res, next)
        local conf = req.body.conf
        local success, data = false, {}
...
API:get("/api_manager/get", function(store)
    return function(req, res, next)
        ...
        local block = false
        local conf = orange_db.get_json("api_manager.conf")
        if conf then
...
API:post("/api_manager/set", function(store)
    return function(req, res, next)
        ...
        if log then
            ngx.log(ngx.INFO, string_format("api_manager-set, dict: %s, key: %s, value: [[%s]], success: %s", dict, key, value, success))
        end
